...
export async function loadReserveFaction(fullPath: string, stage: Stage): Promise<Faction|null> {
    ...
    const generatedResponse = await stage.generator.textGen({
        prompt: `{{messages}}This is preparatory request for structured and formatted game content.` +
            ...
            `Roles involve morally ambiguous or covert activities; many may even be illicit, sexual, or compulsory (essentially human trafficking). ` +
            ...
            `\n\nThe Original Details below describe a character, faction, organization, or setting (${data.name}) from another fantasy city. ` +
            ...
            `#END#\n\n` +
            `Example Response:\n` +
            `NAME: The Crimson Skintraders\n` +
            `DESCRIPTION: A brutish gang of dark elves who take pleasure in collecting only the finest slaves to be ritualistically raped and tortured.\n` +
            `ROLES: Body Slave, Ritualistic Sacrifice, Brood Mare\n` +
            `VISUALSTYLE: Sharp, haphazard architecture with demonic lighting. Members wear skintight leather with revealing cuts. Spaces feature torture implements and restraints.\n` +
            `COLOR: #2a4a7c\n` +
            `FONT: Georgia, serif\n` +
            `#END#`,
        stop: ['#END'],
...
export async function generateFactionModule(faction: Faction, stage: Stage): Promise<string|null> {
    ...
    const generatedResponse = await stage.generator.textGen({
        prompt: `{{messages}}This is preparatory request for structured and formatted game content. The goal is to define a faction-themed module/room for a space station management game. ` +
            ...
            `PURPOSE: A brief summary of the module's function and role on the mansion, as well as how that role might affect the station's slaves or inform skits at this location.\n` +
            ...
            `#END#\n\n` +
            `Example Response:\n` +
            `MODULE NAME: Infernal Engine\n` +
            `PURPOSE: The Infernal Engine provides The Master with additional arcane power to fuel more potent dark spells and rituals\n` +
            `DESCRIPTION: A magical engine thrumming with dark magics.\n` +
            `ROLE NAME: Keeper\n` +
            `ROLE DESCRIPTION: Responsible for maintaining and directing magic generated by the engine.\n` +
            `#END#`,
        stop: ['#END'],
        include_history: true,
        max_tokens: 350,
...
export async function generateFactionModuleImage(faction: Faction, module: ModuleIntrinsic, stage: Stage): Promise<void> {
    ...
    const baseImageUrl = await stage.makeImage({
        prompt: `The detailed interior of an unoccupied medieval fantasy module/room. The design should reflect the following description: ${module.imagePrompt}. ` +
